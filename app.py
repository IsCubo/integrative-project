...
def register_user():
    ...
    try:
        ...
        if not user_id or not user_name or not user_email or not event_id:
            return jsonify({
                "status": "error",
                "message": "❌ Missing required fields (user_id, user_name, user_email, event_id)"
            }), 400
        ...
        # Send confirmation email via SendGrid SMTP
